function CalendarApp(date) {
    ...
    if (!localStorage.getItem('apts')) {
        localStorage.setItem('apts', JSON.stringify([]))
    }

    this.apts = JSON.parse(localStorage.getItem('apts')).map(e => {
        return {
            name: e.name,
            day: new Date(e.day).toString(),
            startTime: new Date(e.startTime).toString(),
            endTime: new Date(e.endTime).toString()
        }
    });
    this.aptDates = JSON.parse(localStorage.getItem('apts')).map(e => new Date(e.day).toString());
...
CalendarApp.prototype.openDayWindow = function(date) {
    ...
    if (day.getDate() < 10) {
        var dayxx = "0" + day.getDate();
    }
    else
    {
        var dayxx = day.getDate();
    }


    var month = parseInt(day.getMonth())+1;

    var date = day.getFullYear() + "-" + month + "-" + dayxx;
...
CalendarApp.prototype.deleteEvent = function(e) {
    const data = JSON.parse(localStorage.getItem('apts'))
    data.splice(e.currentTarget.getAttribute("data-idx"), 1)
    localStorage.setItem('apts', JSON.stringify(data))
...
CalendarApp.prototype.addEvent = function() {
    ...
    const data = localStorage.getItem("apts") ?
        JSON.parse(localStorage.getItem("apts")) : [];
...
CalendarApp.prototype.textOptionLimiter = function(options, input, format) {
    ...
    if (input.indexOf(":") !== -1 && format == "datetime") {
        ...
        } else if (!Number.isInteger(_trailingTime) ||
            _trailingTime < 0 ||
            _trailingTime > 59
        ) {
            return _splitTime[0];
